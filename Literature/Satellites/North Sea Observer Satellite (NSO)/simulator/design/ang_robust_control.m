...
% controller compying with the strongly robust H_inf performance criterion.
...
for k_ang=1:N
  ...
  %pole placement LMISET
  FP{k_ang} = Y*2*pole_lim*eye(n)-Y*A{k_ang}'-W'*B2{k_ang}'-A{k_ang}*Y-B2{k_ang}*W;
  
  %adding LMIs to the lmi set
  lmiset = lmiset + set(FP{k_ang}<0);
  lmiset = lmiset + set(Phi{k_ang}<0);
end

% setup yalmip solver
options = sdpsettings('solver','sedumi','verbose',~run_silent);
solution = solvesdp(lmiset,trace(Y),options);

% retrieves the values from the yalmip variable
Y_feas = double(Y);
W_feas = double(W);

% runs test on solution
if ~run_silent
  checkset(lmiset)
end

% if no solution exists it is printed
if solution.problem
...
%used to analyse the designed controller
if analyse
  for k_ang=1:N
