...
%%%Constants%%%
minB = 30e-6;                        %Tesla over equator 
                                     %http://www.magnetometer.org/mag-magnetic-field.php
Ndet = 364.98e-9;                    %Detumbling torque req.
Ndist = 1599.1e-9;                   %Disturbance torque
required_mm = 2*((Ndet+Ndist)/minB); %required magnetic moment.
...
windings_z = 865;       %coil windings
wire_dia = 0.2e-3;      %wire diameter
...
if(mm_xy < required_mm)                                    %Min. magnetic moment check
    mm_ok = strcat(' FAILED - Need aditional: ',num2str(required_mm-mm_xy),' [Am^2]');
end
...
mm_ok = ' PASSED';
if(mm_z < required_mm)
    mm_ok = strcat(' FAILED - Need aditional: ',num2str(required_mm-mm_z),' [Am^2]');
end
disp(strcat('Magnetic moment check = ',mm_ok))
i_ok = ' PASSED';
