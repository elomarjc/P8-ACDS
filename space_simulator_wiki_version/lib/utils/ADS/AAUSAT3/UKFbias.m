...
tic_s=tic;
    ...
    if t<Ts
        ...
        beta=-(1-alpha^2);
        kappa=0;%3-n_xfull;
        Nrung = 1;   % number of RK4 steps during sample time
        ...
        vbg2=0.001;
        vbg3=0.001;
        P=diag([vq1 vq2 vq3 vw1 vw2 vw3 vbm1 vbm2 vbm3 vbg1 vbg2 vbg3]);
        ...
        vmbg2=0.00000001;
        vmbg3=0.00000001;
        Q=diag([vmq1 vmq2 vmq3 vmw1 vmw2 vmw3 vmbm1 vmbm2 vmbm3 vmbg1 vmbg2 vmbg3]);
        ...
        x_full=[q0; w_meas(1:3); bias_m; bias_g];
        
        % Output first time function is run
        omega_s(1:4,1)=qmul(qmul(q_s_c,[x_full(5:7);0]),qinv(q_s_c));
        residual=zeros(9,1);
        output=[qmul(x_full(1:4),qinv(q_s_c)); omega_s(1:3,1); bias_m(1:3,1); bias_g(1:3,1); residual];
        ...
        for ii=1:n_Xerr
            ...
            if update_mag == 1
                Z_p(4:7,ii)=qmul(qmul(qinv(X_pfull(1:4,ii)),[Mag_I;0]),X_pfull(1:4,ii))+[X_pfull(8:10,ii);0];
            end
            if update_w == 1
        ...
        if (update_sun == 1) && (update_mag == 1)
            x_full=[x_full(1:4); x_err(4:6)+x_pfull(5:7); x_err(7:9)+x_pfull(8:10); x_err(10:12)+x_pfull(11:13)];
        else
            x_full=[x_full(1:4); x_err(4:6)+x_pfull(5:7); x_pfull(8:10); x_err(10:12)+x_pfull(11:13)];
        end
        ...
        output=[qmul(x_full(1:4),qinv(q_s_c)); omega_s(1:3,1); bias_mag_s(1:3,1); bias_gyro_s(1:3,1); residual];
  
    end
tictoc=[tictoc,toc(tic_s)];
assignin('base','tictoc_ukf_bias',tictoc);
end
%***************************************************************************
...
function rungres = f(x, I, Next)
    ...
    rungres = [q_dot; w_dot; bias_mag_dot; bias_gyro_dot];
end
