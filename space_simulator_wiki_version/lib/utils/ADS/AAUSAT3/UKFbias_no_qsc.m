...
tic_s=tic;
    ...
    if t<Ts
        ...
        if update_sun == 1
            residual(1:3)=(Sun_meas-z_p(1:3));
        end
        if update_mag == 1
            % Adding bias requires a renormalisation
            z_p(4:6)=z_p(4:6)./sqrt(z_p(4:6)'*z_p(4:6));
            residual(4:6)=(Mag_meas-z_p(4:6));
        end
        if update_w == 1
            residual(7:9)=(w_meas-z_p(7:9));
        end
        ...
        output=[x_full(1:4); x_full(5:7); x_full(8:10)*temp_norm1*10^9; x_full(11:13); residual];
